...
static struct  TypedIdentList *formals() {
  ...
  // TODO
  struct TypedIdent *node1 = newTypedIdent();

  next();
  if(!is_token(IDENT))
  	return NULL;
  ensure_token(IDENT);
  node1->identifier = token()->identifier;
...
static struct  Statement *statement() {
  ...
  if (is_token(IDENT)) {
    ...
    while(!is_token(END))
    {
    	struct Statement *node2 = statement();
    	next();
    	addStatement(list, node2);
    }
    previous();
...
static struct  Expression *simpleexpr() {
  ...
  if (is_termop()) {
    ...
    previous();
    return left; // be sure to change this to return a node!
    } 
}
